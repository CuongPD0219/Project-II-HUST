...
% =========================================================================
    ...
    calc_a = @(r, theta_deg) exp(-1j * 2 * pi * ...
        (sqrt(r^2 + (delta_n * d).^2 - 2 * r * (delta_n * d) * sin(deg2rad(theta_deg))) - r) / lam).';
    ...
    for k = 1:K
        ...
        % suy hao không gian tự do (Free Space Path Loss Amplitude)
        % Công thức: 
        ...
        h_NLoS = zeros(Nt, 1);
        for l = 1:P
            r_l = r_k + (rand-0.5)*2;                       %phan tan ngau nhien trong khoang 1 m [-0.5, 0.5]
            theta_l = pos_users(k,2) + (rand-0.5)*10;       % phan tan goc +- 5 do   [-0.5, 0.5]
